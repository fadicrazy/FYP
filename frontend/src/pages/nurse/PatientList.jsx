...
export default function PatientList() {
  ...
  useEffect(() => { patientsAPI.getAll().then(r => { setPatients(r.data || []); setLoading(false); }).catch(() => setLoading(false)); }, []);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="table-wrapper">
        <table>
          <thead><tr><th>Patient</th><th>Age</th><th>Gender</th><th>Blood Group</th><th>Address</th><th>Contact</th></tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">Loading...</td></tr> :
            filtered.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">No patients found</td></tr> :
            filtered.map((p, i) => (
              <tr key={i}>
                <td>
